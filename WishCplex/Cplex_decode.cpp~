...
//to use external offset, i.e., the b part of Ax=b
...
vector <int> parseInput()
{
  ...
  const char* recChars = received.c_str();
...
void parseParityArgs(int & argc, char **argv)
{
  ...
  for (int argIndex=1; argIndex < argc; ++argIndex) {
    if ( !strcmp(argv[argIndex], "-rec") ) {
      argIndex++;
      received = string(argv[argIndex]);
    }
    else if ( !strcmp(argv[argIndex], "-skipelim") ) {
      elim=false;
    }
    else if ( !strcmp(argv[argIndex], "-offset") ) {
      externalOffset=true;
    }
    ...
    else if ( !strcmp(argv[argIndex], "-nbvar") ) {
      argIndex++;
      nbvar = (IloInt)atol( argv[argIndex] );
    }
...
int main(int argc, char **argv)
{
  ...
  try {
    ...
    IloModel model(env);
    cout<<"parity number:"<<parity_number<<endl;
...
if (!A.empty())
{
	if(!elim)
	{
		//save a copy of A for future use
	        vector <vector <bool> > Aorig(A);
		row_echelon(A);
		A.swap(Aorig);
		print_matrix(A);
	}
...
if (!A.empty())
{
	...
	//		IloNumVarArray alphas(env);
			...
			for (size_t k = 0; k<= 2* (size_t) floor(f/2);k=k+2)
				...
				printf(name, "alpha_%d_%d", (int) j, (int) k);
