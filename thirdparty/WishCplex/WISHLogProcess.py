...
def logsumexp(M, maxdepth):
    ...
    # The following lines do not match the fomula in the paper exactly - shih-ming
    # W = 0.0
    # for i in range(0, maxdepth + 1):
    #     W = W + math.exp(M[i] * math.log(10) + (i - 1) * math.log(2) - curmax)
    W = math.exp(M[0] * math.log(10) - curmax)
    for i in range(1, maxdepth + 1):
        W = W + math.exp(M[i] * math.log(10) + (i - 1) * math.log(2) - curmax)
...
def process_logs(folder):
        ...
        if files.endswith(".LOG"):
            ...
            if m is not None:
                ...
                with open(files, 'r') as f:
                    ...
                    lines = f.read()
                    entries = re.search(
                        "Optimum: (\d+) log10like: ([-+]?(\d+(\.\d*)?|\.\d+)([eE][-+]?\d+)?)", lines)
                    if entries is not None:
                        w.append([i, t, float(entries.group(2)), True])
                        entriesSol = re.search(
                            "Optimal solution: (\d+)", lines)
                        if entriesSol is not None:
                            Samples.append(
                                [i, t, float(entries.group(2)), True, entriesSol.group(1)])
                    else:
                        entries2 = re.findall(
                            "New solution: (\d+) log10like: ([-+]?(\d+(\.\d*)?|\.\d+)([eE][-+]?\d+)?)", lines)
                        if entries2:
                            # find last entry
                            # print entries2[-1]
                            w.append([i, t, float(entries2[-1][1]), False])
                            entriesSol = re.findall(
                                "Current solution: (\d+)", lines)
                            if entriesSol:
                                # print entriesSol[-1]
                                Samples.append(
                                    [i, t, float(entries2[-1][1]), False, entriesSol[-1]])
                        else:
                            # Inconsistency detected!
                            entries3 = re.search(
                                "No solution in (\d+) backtracks and", lines)
                            entries4 = re.search(
                                "Inconsistency detected!", lines)
                            if (entries3 is not None or entries4 is not None):
                                # nothing = 34
                                # problem is unsat
                                w.append([i, t, float("-inf"), True])
                            else:
    ...
    for i in range(maxdepth + 1):
        results = [e[2] for e in w if e[0] == i]
        resultsOpt = [e[2] for e in w if ((e[0] == i) and (e[3] is True))]
        resultsSubOpt = [e[2] for e in w if ((e[0] == i) and (e[3] is False))]
        if len(results) > 0:
            x = x + results
        ...
        if files.endswith(".LOG"):
            ...
            if m is not None:
                ...
                with open(files, 'r') as f:
                    ...
                    entries = re.search(
                        "Solution value log10lik = ([-+]?(\d+(\.\d*)?|\.\d+)([eE][-+]?\d+)?)", lines)

                    # The following line seems to be a bug -shih-ming
                    ...
                    if entries is not None:
                        ...
                        if entriesSol is not None:
                            Samples.append(
                                [i, t, float(entries.group(1)), True, entriesSol.group(1)])
                    else:
                        entries2 = re.findall(
                    ...
                    # Optimum: 91499957 log10like: -3.97379 prob: 0.00010622 in
                    # 299 backtracks and 600 nodes and 0.08 seconds.
            else:
                print "Error " + files
